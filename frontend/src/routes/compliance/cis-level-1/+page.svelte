...
import { loadThresholds, scoreColor, scoreLabel } from '$lib/thresholds.svelte.js';
	...
	const profileColor = '#10b981';
	const profileInfo = {
		icon: 'solar:shield-check-bold',
		title: 'CIS Level 1',
		subtitle: 'Basic security hardening — 58 checks across 7 categories',
		desc: 'CIS Level 1 scans foundational controls every production server should implement. These are the minimum-security configuration recommendations from the Center for Internet Security (CIS) Benchmark.',
	};

		const categoryMeta = {
		ssh: { label: 'SSH', icon: '🔒', color: '#3b82f6', checks: 10 },
		kernel: { label: 'Kernel', icon: '⚙️', color: '#8b5cf6', checks: 8 },
		filesystem: { label: 'Filesystem', icon: '📁', color: '#f59e0b', checks: 6 },
		users: { label: 'Users & Groups', icon: '👥', color: '#fb923c', checks: 6 },
		services: { label: 'Services', icon: '⚡', color: '#ef4444', checks: 4 },
		network: { label: 'Network', icon: '🌐', color: '#10b981', checks: 3 },
		logging: { label: 'Logging', icon: '📋', color: '#c084fc', checks: 2 },
	...
	onMount(async () => {
		...
		try {
			...
			checks = (data.checks || []).filter(c => c.cis_level === 1 || c.cis_level === 0);
		} catch (e) {
			error = e.message;
		} finally {
			loading = false;
	...
	async function loadHistory(pg) {
		...
		historyLoading = true;
		try {
			const resp = await api.compliance.globalHistory({ scan_type: 'CIS Level 1', page: historyPage, limit: historyLimit });
...
<div class="page-container">
	...
	<div class="flex items-center gap-2 text-sm mb-4" style="color: var(--color-text-muted);">
		...
		<span>/</span>
		<span style="color: var(--color-text);">CIS Level 1</span>
	</div>
		...
		<div class="flex items-start gap-4 flex-wrap">
			...
			<div class="flex-1 min-w-0">
				...
				<div class="flex flex-wrap items-center gap-3 mt-2">
					<span class="text-xs px-2.5 py-1 rounded-full font-medium" style="background: rgba(16,185,129,0.12); color: {profileColor};">
						{totalChecks} checks
					</span>
				...
				<div class="flex items-center gap-2 mt-3">
					...
					<button onclick={runScan} disabled={scanning || !selectedServerId} class="btn-primary flex items-center gap-1.5 text-xs py-1.5">
						...
						{scanning ? 'Scanning...' : 'Run CIS L1 Scan'}
					</button>
					{#if scanMessage}
	...
	{:else}
			...
			<div class="card !p-0 overflow-hidden">
				...
				{:else}
					<div class="flex flex-col items-center py-10 text-center">
						...
						<p class="text-sm" style="color: var(--color-text-secondary);">No scan history yet for CIS Level 1</p>
		...
		<div class="mt-6 text-center">
			<p class="text-xs" style="color: var(--color-text-muted);">
				...
				Checks may vary by distribution and version.
			</p>
		</div>
	{/if}
</div>
