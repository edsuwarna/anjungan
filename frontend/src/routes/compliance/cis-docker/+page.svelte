<script>
	...
	const profileInfo = {
		icon: 'solar:box-bold',
		title: 'CIS Docker',
		subtitle: 'Docker daemon & host configuration audit — 22 checks across 6 categories',
		desc: 'CIS Docker Benchmark scans the Docker daemon host configuration, file permissions, container runtime settings, network security, authorization, and image registry configuration. These checks follow the CIS Docker Benchmark v1.6 standard.',
	};

	const categoryMeta = {
		'daemon-config': { label: 'Daemon Configuration', icon: '⚙️', color: '#06b6d4', checks: 5 },
		'daemon-files': { label: 'Daemon Files', icon: '📁', color: '#8b5cf6', checks: 3 },
		'container-runtime': { label: 'Container Runtime', icon: '🚀', color: '#f59e0b', checks: 5 },
		'container-network': { label: 'Container Network', icon: '🌐', color: '#3b82f6', checks: 4 },
		'container-auth': { label: 'Authorization & Access', icon: '🔑', color: '#ef4444', checks: 2 },
		'images-registry': { label: 'Images & Registry', icon: '📦', color: '#fb923c', checks: 3 },
	};
	...
	onMount(async () => {
		...
		try {
			...
			checks = (data.checks || []).filter(c => c.category === 'daemon-config' || c.category === 'daemon-files' || c.category === 'container-runtime' || c.category === 'container-network' || c.category === 'container-auth' || c.category === 'images-registry');
		} catch (e) {
			error = e.message;
		} finally {
			loading = false;
	...
	async function runDockerScan() {
		...
		try {
			await api.compliance.scanDocker(selectedServerId);
	...
	async function loadHistory(pg) {
		...
		historyLoading = true;
		try {
			const resp = await api.compliance.globalHistory({ scan_type: 'CIS Docker', page: historyPage, limit: historyLimit });
...
<div class="page-container">
	...
	<div class="flex items-center gap-2 text-sm mb-4" style="color: var(--color-text-muted);">
		...
		<span>/</span>
		<span style="color: var(--color-text);">CIS Docker</span>
	</div>

	<!-- Hero -->
	<div class="card !p-5 mb-5" style="border-left: 4px solid {profileColor};">
		<div class="flex items-start gap-4 flex-wrap">
			<div class="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style="background: rgba(6,182,212,0.12);">
			...
			<div class="flex-1 min-w-0">
				...
				<div class="flex flex-wrap items-center gap-3 mt-2">
					<span class="text-xs px-2.5 py-1 rounded-full font-medium" style="background: rgba(6,182,212,0.12); color: {profileColor};">
						{totalChecks} checks
					</span>
				...
				<div class="flex items-center gap-2 mt-3">
					...
					{/if}
					<button onclick={runDockerScan} disabled={scanning || !selectedServerId} class="btn-primary flex items-center gap-1.5 text-xs py-1.5">
	...
	{:else}
			...
			<div class="card !p-0 overflow-hidden">
				...
				{:else}
					<div class="flex flex-col items-center py-10 text-center">
						...
						<p class="text-sm" style="color: var(--color-text-secondary);">No scan history yet for CIS Docker</p>
						<p class="text-xs mt-1" style="color: var(--color-text-muted);">Run a Docker scan from a server's Compliance tab to see results here.</p>
					</div>
				{/if}
			</div>
		</div>
		...
		<div class="mt-6 text-center">
			<p class="text-xs" style="color: var(--color-text-muted);">
				Based on <strong>CIS Docker Benchmark v1.6</strong>.
				Checks cover Docker daemon configuration, file permissions, container runtime, networking, authorization, and image registry.
			</p>
		</div>
	{/if}
</div>
