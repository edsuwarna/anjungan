...
<div class="page-container">
	...
	{:else if project}
		<nav class="breadcrumb">
			...
			<span class="crumb-sep">›</span>
			<span class="current">Containers</span>
		</nav>

		<div class="flex items-start justify-between flex-wrap gap-3 mb-6">
			<div>
				<h1 class="page-title">Containers</h1>
				<p class="page-subtitle mt-1">Containers scoped to {project.name}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border px-6 py-12 text-center" style="background-color: var(--color-card); border-color: var(--color-border-light);">
			...
			<h3 class="mb-2 text-base font-semibold" style="color: var(--color-text);">No Containers Yet</h3>
			<p class="mx-auto max-w-md text-sm" style="color: var(--color-text-secondary);">
				This project doesn't have any containers yet. Add your first container to get started.
			</p>
			<button onclick={() => goto('/containers')} class="btn-primary mt-6 inline-flex items-center gap-2 text-sm">
