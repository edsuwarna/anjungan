	...
	{:else if project}
		...
		<div class="flex items-start justify-between flex-wrap gap-3 mb-6">
			<div>
				...
				<p class="page-subtitle mt-1">Notification targets scoped to {project.name}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border px-6 py-12 text-center" style="background-color: var(--color-card); border-color: var(--color-border-light);">
			<Icon icon="solar:bell-bold" class="mb-3 inline-block h-12 w-12" style="color: var(--color-text-muted);" />
			<h3 class="mb-2 text-base font-semibold" style="color: var(--color-text);">No Notifications Yet</h3>
			<p class="mx-auto max-w-md text-sm" style="color: var(--color-text-secondary);">
				This project doesn't have any notification targets yet. Add your first notification target to get started.
			</p>
			<button onclick={() => goto('/notifications')} class="btn-primary mt-6 inline-flex items-center gap-2 text-sm">
