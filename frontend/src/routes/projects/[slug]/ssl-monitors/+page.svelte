...
<div class="page-container">
	...
	{:else if project}
		...
		<nav class="breadcrumb">
			...
			<span class="crumb-sep">›</span>
			<span class="current">SSL Monitors</span>
		</nav>

		<!-- Header -->
		<div class="flex items-start justify-between flex-wrap gap-3 mb-6">
			<div>
				<h1 class="page-title">SSL Monitors</h1>
				<p class="page-subtitle mt-1">SSL monitors scoped to {project.name}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border px-6 py-12 text-center" style="background-color: var(--color-card); border-color: var(--color-border-light);">
			...
			<h3 class="mb-2 text-base font-semibold" style="color: var(--color-text);">Project-scoped SSL Monitors</h3>
			<p class="mx-auto max-w-md text-sm" style="color: var(--color-text-secondary);">
				SSL monitor list for <strong>{project.name}</strong> will appear here once the scoped API client is integrated.
				The backend endpoints at <code class="text-xs">/api/v1/projects/{slug}/ssl-monitors</code> are functional — the frontend integration will follow.
			</p>
