	...
	{:else if project}
		...
		<div class="flex items-start justify-between flex-wrap gap-3 mb-6">
			<div>
				...
				<p class="page-subtitle mt-1">Servers scoped to {project.name}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border px-6 py-12 text-center" style="background-color: var(--color-card); border-color: var(--color-border-light);">
			...
			<h3 class="mb-2 text-base font-semibold" style="color: var(--color-text);">Project-scoped Servers</h3>
			<p class="mx-auto max-w-md text-sm" style="color: var(--color-text-secondary);">
				Server list for <strong>{project.name}</strong> will appear here once the scoped API client is integrated.
				The backend endpoints at <code class="text-xs">/api/v1/projects/{slug}/servers</code> are functional — the frontend integration will follow.
			</p>
