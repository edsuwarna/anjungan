...
<div class="page-container">
	...
	{:else if project}
		<nav class="breadcrumb">
			...
			<span class="crumb-sep">›</span>
			<span class="current">SSH Keys</span>
		</nav>

		<div class="flex items-start justify-between flex-wrap gap-3 mb-6">
			<div>
				<h1 class="page-title">SSH Keys</h1>
				<p class="page-subtitle mt-1">SSH keys scoped to {project.name}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border px-6 py-12 text-center" style="background-color: var(--color-card); border-color: var(--color-border-light);">
			<Icon icon="solar:key-minimalistic-bold" class="mb-3 inline-block h-12 w-12" style="color: var(--color-text-muted);" />
			<h3 class="mb-2 text-base font-semibold" style="color: var(--color-text);">No SSH Keys Yet</h3>
			<p class="mx-auto max-w-md text-sm" style="color: var(--color-text-secondary);">
				This project doesn't have any SSH keys yet. Add your first SSH key to get started.
			</p>
			<button onclick={() => goto('/ssh-keys')} class="btn-primary mt-6 inline-flex items-center gap-2 text-sm">
